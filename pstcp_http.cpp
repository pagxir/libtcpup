...
class pstcp_http {
   	public:
		pstcp_http(struct tcpcb * tp, const char * path);
		~pstcp_http();
...
pstcp_http::pstcp_http(struct tcpcb * tp, const char * path)
...
int pstcp_http::create_response_stream(const char * path0, int range, size_t length, size_t offset)
{
	...
	switch (range) {
		case 0:
			sprintf(m_rbuf, resp200_template,
				file_len, get_file_type(path0));
			m_rlen = strlen(m_rbuf);
			break;
...
int pstcp_http::run(void)
{
	...
	if (m_roff == m_rlen) {
		...
		if ((m_flags & test_flags) == TF_RCV_HEADER) {
			m_rlen = sizeof(m_rbuf); //fread(m_rbuf, 1, sizeof(m_rbuf), m_file);
			if (m_rlen == 0)
				m_flags |= TF_EOF1;
			m_roff = 0;
		}
...
extern "C" void pstcp_channel_forward(u_long addr, u_short port)
{
}
